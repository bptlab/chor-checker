...
export async function checkModel(xml: string, property: string): Promise<Object> {
  ...
  await fs.copy('./environment', folder);
  console.log(id, 'Copied execution files');

  const tla = await generateTLA(xml, property);
  console.log(id, 'Generated TLA');

  await fs.outputFile(folder + '/Choreography.tla', tla);
  console.log(id, 'TLA written to file');

  // now the interesting part: running TLC
  let buffer: Buffer;
  try {
    buffer = child.execSync('java -classpath tla2tools.jar tlc2.TLC -deadlock -nowarning SemanticsContinual', { cwd: folder });
  } catch (error) {
